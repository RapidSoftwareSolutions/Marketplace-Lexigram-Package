...
$app->post('/api/Lexigram/search', function ($request, $response) {
    ...
    $url = $settings['apiUrl'] . "/lexigraph/search/";

    $headers['Authorization'] = 'Bearer ' . $postData['args']['apiKey'];
    $headers['Content-type'] = 'application/json';

    $params['q'] = $postData['args']['query'];
    if (isset($postData['args']['limit']) && strlen($postData['args']['limit']) > 0) {
        $params['limit'] = $postData['args']['limit'];
    }
